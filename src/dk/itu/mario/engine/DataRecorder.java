...
public class DataRecorder {
	...
	private int completionTime; // counts only the current run on the level,
								// excluding death games
	private int totalTime; // sums all the time, including from previous games
							// if player died
	 ...
	 */
	private int totalRightTime, totalLeftTime, startRightTime, endRightTime,
			startLeftTime, endLeftTime;
	 ...
	 */
	private int blocksEmptyDestroyed, blocksCoinDestroyed,
	...
	public void reset() {
		kills = new int[7];
		deaths = new int[10]; // added one for the hole death and one for time
								// death and one for shell
	...
	public void endRightMoveRecord() {
		...
		detailedLog += "RightMove: StTime = " + startRightTime + " EdTime = "
				+ totalRightTime;
		detailedLog += "\n";
	}
	...
	public void endLeftMoveRecord() {
		...
		detailedLog += "LeftMove: StTime = " + startLeftTime + " EdTime = "
				+ totalLeftTime;
		detailedLog += "\n";
	...
	public void endDuckRecord() {
		if (levelScene.mario.ducking) {
			...
			detailedLog += "Duck: StTime = " + startDuckTime + " EdTime = "
					+ endDuckTime;
			detailedLog += "\n";
	...
	public void endLittleRecord() {
		if (littleRecording) {
			...
			System.out.println("LITTLE END: " + endLittleTime);
			detailedLog += "LittleState: StTime = " + startLittleTime
					+ " EdTime = " + endLittleTime;
			detailedLog += "\n";
		}
	}
	...
	public void endLargeRecord() {
		...
		detailedLog += "LargeState: StTime = " + startLargeTime + " EdTime = "
				+ endLargeTime;
		detailedLog += "\n";
	...
	public void endFireRecord() {
		...
		detailedLog += "FireState: StTime = " + startFireTime + " EdTime = "
				+ endFireTime;
		detailedLog += "\n";
	}
	...
	public void endRunningRecord() {
		if (levelScene.mario.running) {
			...
			detailedLog += "RunState: StTime = " + startRunTime + " EdTime = "
					+ endRunTime;
			detailedLog += "\n";
	...
	public void fireKillRecord(Sprite sprite) {
		...
		if (sprite instanceof FlowerEnemy) {
			detailedLog += "FireKill:  EnemyType = FlowerEnemy  time = "
					+ (2982 - levelScene.timeLeft);
			detailedLog += "\n";
		} else if (sprite instanceof BulletBill) {// cannon shot
		} else if (sprite instanceof Shell) {
		} else if (sprite instanceof Enemy) {
			Enemy enemy = (Enemy) sprite;
			detailedLog += "FireKill:  EnemyType =" + enemy.type + "time = "
					+ (2982 - levelScene.timeLeft);
			detailedLog += "\n";
		}
		fireKills++;
	...
	public void shellKillRecord(Sprite sprite) {
		killRecord(sprite);
		if (sprite instanceof FlowerEnemy) {
			detailedLog += "ShellKill:  EnemyType = FlowerEnemy time = "
					+ (2982 - levelScene.timeLeft);
			detailedLog += "\n";
		} else if (sprite instanceof BulletBill) {// cannon shot
		} else if (sprite instanceof Shell) {
			detailedLog += "ShellKill:  EnemyType = Turtle time = "
					+ (2982 - levelScene.timeLeft);
			detailedLog += "\n";
		} else if (sprite instanceof Enemy) {
			Enemy enemy = (Enemy) sprite;
			detailedLog += "ShellKill:  EnemyType = " + enemy.type + " time = "
					+ (2982 - levelScene.timeLeft);
			detailedLog += "\n";
		}
	...
	public void killStompRecord(Sprite sprite) {
		killRecord(sprite);
		if (sprite instanceof FlowerEnemy) {
			detailedLog += "StompKill:  EnemyType = FlowerEnemy time = "
					+ (2982 - levelScene.timeLeft);
			detailedLog += "\n";

		} else if (sprite instanceof BulletBill) {// cannon shot
			detailedLog += "StompKill:  EnemyType = BulletBill time = "
					+ (2982 - levelScene.timeLeft);
			detailedLog += "\n";
		} else if (sprite instanceof Shell) {
			// levelScene.levelRecorder.enemyR.interact.add(new
			// DataEntry(EnemyRecorder.GREEN_TURTLE, EnemyRecorder.UNLEASHED,
			// 2982 - levelScene.timeLeft,x,y));
		} else if (sprite instanceof Enemy) {
			Enemy enemy = (Enemy) sprite;
			detailedLog += "StompKill:  EnemyType = " + enemy.type + " time = "
					+ (2982 - levelScene.timeLeft);
			detailedLog += "\n";
	...
	public void killRecord(Sprite sprite) {
		// something goes wrong with the type of the flower enemy, this is
		// special case
		if (sprite instanceof FlowerEnemy) {
	...
	public void blockCoinDestroyRecord() {
		detailedLog += "BlockCoinDestroy:  time = "
				+ (2982 - levelScene.timeLeft);
		detailedLog += "\n";
		blocksCoinDestroyed++;
	}
	...
	public void blockPowerDestroyRecord() {
		detailedLog += "BlockPowerDestroy:  time = "
				+ (2982 - levelScene.timeLeft);
		detailedLog += "\n";
		blocksPowerDestroyed++;
	}
	...
	public void recordJumpLand() {
		if (isInAir) {
			...
			detailedLog += "Jump:  StTime = " + startJumpTime + " EdTime = "
					+ endJumpTime;
			detailedLog += "\n";
		}
	}
	...
	public void printAll() {
		...
		System.out.println("total fire: " + convertTime(totalFireTime)
				+ " total large: " + convertTime(totalLargeTime)
				+ " total little: " + convertTime(totalLittleTime));
	}

	private void printSwitching() {
		printStart("Switch Variables");
		System.out.println("Time Spent Moving Right: "
				+ convertTime(totalRightTime)
				+ " ("
				+ Math.round((double) convertTime(totalRightTime)
						/ (double) convertTime(totalTime) * (double) 100)
				+ "%)");
		System.out.println("Time Spent Moving Left: "
				+ convertTime(totalLeftTime)
				+ " ("
				+ Math.round((double) convertTime(totalLeftTime)
						/ (double) convertTime(totalTime) * (double) 100)
				+ "%)");
		System.out
				.println("Time Spent Standing Still: "
						+ (convertTime(totalTime) - convertTime(totalLeftTime) - convertTime(totalRightTime))
						+ " ("
						+ Math.round((double) (convertTime(totalTime)
								- convertTime(totalLeftTime) - convertTime(totalRightTime))
								/ (double) convertTime(totalTime)
								* (double) 100) + "%)");
	...
	private void printJump() {
		...
		System.out.println("Time Spent Jumping: "
				+ convertTime(totalJumpTime)
				+ " ("
	...
	private void printRun() {
		...
		System.out.println("Time Spent Running: "
				+ convertTime(totalRunTime)
				+ " ("
				+ Math.round((double) convertTime(totalRunTime)
						/ (double) convertTime(totalTime) * 100) + "%)");
		printEnd();
	}
	...
	private void printDuck() {
		...
		System.out.println("Time Spent Ducking: "
				+ convertTime(totalDuckTime)
				+ " ("
				+ Math.round((double) convertTime(totalDuckTime)
						/ (double) convertTime(totalTime) * (double) 100)
				+ "%)");
		printEnd();
	}
	...
	public void fillGamePlayMetrics(Level level) {
		...
		gpm.totalTime = getTotalTime();// //sums all the time, including from
										// previous games if player died
		...
		gpm.enemyKillByFire = getKillsFire();// Number of Kills by Shooting
												// Enemy
		gpm.enemyKillByKickingShell = getKillsShell();// Number of Kills by
														// Kicking Shell on
														// Enemy
		gpm.totalEnemies = level.ENEMIES;

		gpm.totalTimeLittleMode = getTotalLittleTime();
		gpm.totalTimeLargeMode = getTotalLargeTime();// Time Spent Being Large
														// Mario
		gpm.totalTimeFireMode = getTotalFireTime();// Time Spent Being Fire
													// Mario
		gpm.timesSwichingPower = getSwitchedPower();// Number of Times Switched
													// Between Little, Large or
													// Fire Mario
		gpm.aimlessJumps = J();// aimless jumps
		gpm.percentageBlocksDestroyed = nb();// percentage of all blocks
												// destroyed
		gpm.percentageCoinBlocksDestroyed = ncb();// percentage of coin blocks
													// destroyed
		gpm.percentageEmptyBlockesDestroyed = neb();// percentage of empty
													// blocks destroyed
		gpm.percentagePowerBlockDestroyed = np();// percentage of power blocks
													// destroyed
		gpm.timesOfDeathByFallingIntoGap = dg();// number of death by falling
												// into a gap
	...
	public double ks() {
		if (getKillsStomp() + getKillsFire() == 0) {
			...
			return (double) getKillsStomp()
					/ (double) (getKillsStomp() + getKillsFire());
	}
	...
	public double kf() {
		if (getKillsFire() + getKillsStomp() == 0)
			...
			return (double) getKillsFire()
					/ (double) (getKillsFire() + getKillsStomp());
	}
	 ...
	 */
	public double J() {
		return getTimesJumped() - getKillsStomp() - getBlocksEmptyDestroyed()
				- getBlocksCoinDestroyed() - getBlocksPowerDestroyed();
	}
